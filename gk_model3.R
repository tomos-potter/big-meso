...
competitionSurface <- nimbleFunction(
  run = function(H = double(0), population = double(0),  
                 philength.focal = double(0), philength.comp = double(0),
                 rholength.focal = double(0), rholength.comp = double(0),
                 iota = double(0), eta = double(0),
                 density.focal = double(0), le.comp = double(1), le.focal = double(1))
  {
    # numNumber <- population * (sum( ((exp(iota + philength.comp * le.comp - philength.focal * le.focal) ) *
    #                            exp((-(eta + rholength.comp * le.comp - rholength.focal * le.focal)^2) / (2*H^2) ))[1:density.focal]
    # ) ) / 1.4
    
    numNumber <- population * (sum( ((exp(0 + philength.comp * le.comp - philength.focal * le.focal) ) *
                                exp((-(0 + 0 * le.comp - 0 * le.focal)^2) / (2*H^2) ))[1:density.focal]
    ) ) / 1.4
